...
throw_obj(struct obj *obj, int shotlimit)
{
    ...
    if (!uarmg && obj->otyp == CORPSE && touch_petrifies(&mons[obj->corpsenm])
        && !Stone_resistance) {
        You(_("throw %s with your bare %s."),
...
hurtle_step(genericptr_t arg, coordxy x, coordxy y)
{
    ...
    if (is_pool(x, y) && !u.uinwater) {
        if (is_waterwall(x, y) || !(Levitation || Flying || Wwalking)) {
            ...
            Norep(_("You move over %s."), an(is_moat(x, y) ? _("moat") : _("pool")));
        }
    ...
    if ((ttmp = t_at(x, y)) != 0) {
        if (stopping_short) {
            ...
            pline(_("The ground vibrates as you pass it."));
...
toss_up(struct obj *obj, boolean hitsroof)
{
 ...
 petrify:
        ...
        gt.thrownobj = 0;
        if (!harmless)
            losehp(dmg, _("falling object"), KILLED_BY_AN);
    }
    return TRUE;
}
    ...
    struct obj *oldslot) /* for thrown-and-return used with !fixinv */
{
    ...
    if ((u.dx || u.dy || (u.dz < 1))
        ...
        && !Is_airlevel(&u.uz)) {
        You(_("have so little stamina, %s drops from your grasp."),
            the(xname(obj)));
    ...
    if (u.uswallow) {
        ...
        if (is_ammo(obj)) {
            if (ammo_and_launcher(obj, uwep)) {
                ...
                range /= 2;
                pline(_("You aren't wielding %s, so you throw your %s by %s."),
...
throw_gold(struct obj *obj)
{
    ...
    if (u.dz) {
        if (u.dz < 0 && !Is_airlevel(&u.uz) && !Underwater
            ...
            /* some self damage? */
            if (uarmh)
                pline(_("Fortunately, you are wearing %s!"),
