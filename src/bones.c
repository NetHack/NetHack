...
STATIC_DCL boolean fixuporacle(struct monst *);
...
void
drop_upon_death(struct monst *mtmp /* monster if hero turned into one (other than ghost) */, struct obj *cont /* container if hero is turned into a statue */, int x, int y)
{
    struct obj *otmp;
