          ...
          int type)
{
    ...
    if (xtime > 0L) {
        ...
        if (u.usick_type) { /* only partly cured */
            if (talk)
                You_feel(_("somewhat better."));
            set_itimeout(&Sick, Sick * 2); /* approximation */
        } else {
            if (talk)
                You_feel(_("cured.  What a relief!"));
...
make_blinded(long xtime, boolean talk)
{
    ...
    if (can_see_now && !u_could_see) { /* regaining sight */
        if (talk) {
            if (Hallucination)
                pline(_("Far out!  Everything is all cosmic again!"));
            else
                You(_("can see again."));
        }
    } else if (old && !xtime) {
    ...
    if (u_could_see && !can_see_now) { /* losing sight */
        if (talk) {
            if (Hallucination)
                pline(_("Oh, bummer!  Everything is dark!  Help!"));
            else
                pline(_("A cloud of darkness falls upon you."));
        }
...
ghost_from_bottle(void)
{
    ...
    if (Blind) {
        pline(_("As you open the bottle, %s emerges."), something);
        return;
    }
...
dodrink(void)
{
    ...
    if (Strangled) {
        pline(_("If you can't breathe air, how can you drink liquid?"));
        return ECMD_OK;
    }
...
peffect_hallucination(struct obj *otmp)
{
    ...
    if ((otmp->blessed && !rn2(3)) || (!otmp->cursed && !rn2(6))) {
        You(_("perceive yourself..."));
        display_nhwindow(WIN_MESSAGE, FALSE);
        enlightenment(MAGICENLIGHTENMENT, ENL_GAMEINPROGRESS);
        Your(_("awareness re-normalizes."));
        exercise(A_WIS, TRUE);
    }
}
...
peffect_water(struct obj *otmp)
{
    if (!otmp->blessed && !otmp->cursed) {
        pline(_("This tastes like %s."), hliquid("water"));
        u.uhunger += rnd(10);
        newuhs(FALSE);
        return;
    }
    ...
    if (mon_hates_blessings(&gy.youmonst) /* undead or demon */
        ...
        if (otmp->blessed) {
            ...
            exercise(A_CON, FALSE);
            if (ismnum(u.ulycn)) {
                Your(_("affinity to %s disappears!"),
...
peffect_invisibility(struct obj *otmp)
{
    ...
    if (otmp->cursed) {
        pline(_("For some reason, you feel your presence is known."));
        aggravate();
...
peffect_see_invisible(struct obj *otmp)
{
    ...
    if (msg && !Blind) {  /* Blind possible if polymorphed */
        You(_("can see through yourself, but you are visible!"));
        gp.potion_unkn--;
    }
}
...
{
    if (Free_action) {
        You(_("stiffen momentarily."));
    } else {
        if (Levitation || Is_airlevel(&u.uz) || Is_waterlevel(&u.uz))
            You(_("are motionlessly suspended."));
        else if (u.usteed)
...
peffect_sickness(struct obj *otmp)
{
    ...
    if (otmp->blessed) {
        ...
        if (Poison_resistance)
            pline(_("(But in fact it was biologically contaminated %s.)"),
                  fruitname(TRUE));
        if (Role_if(PM_HEALER)) {
            pline(_("Fortunately, you have been immunized."));
        } else {
    ...
    if (Hallucination) {
        You(_("are shocked back to your senses!"));
...
peffect_speed(struct obj *otmp)
{
    ...
    if (is_speed && !otmp->cursed && !(HFast & INTRINSIC)) {
        Your(_("quickness feels very natural."));
        HFast |= FROMOUTSIDE;
    }
}
        ...
        if (on_lvl_1 ? u.uhave.amulet : Can_rise_up(u.ux, u.uy, &u.uz)) {
            ...
            if (on_lvl_1) {
                ...
                if (on_level(&newlevel, &u.uz)) {
                    pline(_("It tasted bad."));
                    return;
                }
            }
            You(_("rise up, through the %s!"), ceiling(u.ux, u.uy));
...
peffect_gain_energy(struct obj *otmp)
{
    ...
    if (otmp->cursed)
        You_feel(_("lackluster."));
    else
        pline(_("Magical energies course through your body."));
...
peffect_oil(struct obj *otmp)
{
    ...
    if (otmp->lamplit) {
        if (likes_fire(gy.youmonst.data)) {
             ...
             */
            You(_("burn your %s."), body_part(FACE));
            /* fire damage */
        ...
        pline(_("This tastes like castor oil."));
    } else {
...
potionhit(struct monst *mon, struct obj *obj, int how)
{
 ...
 do_healing:
            ...
            if (mon->mhp < mon->mhpmax) {
                ...
                if (canseemon(mon))
                    pline(_("%s looks sound and hale again."), Monnam(mon));
            }
            if (cureblind)
        ...
        case POT_SICKNESS:
            ...
            if (dmgtype(mon->data, AD_DISE)
                ...
                || resists_poison(mon)) {
                if (canseemon(mon))
                    pline(_("%s looks unharmed."), Monnam(mon));
                break;
            }
 do_illness:
            if (mon->mhp > 2) {
                mon->mhp /= 2;
                if (canseemon(mon))
                    pline(_("%s looks rather ill."), Monnam(mon));
            }
            break;
        case POT_CONFUSION:
        case POT_BOOZE:
        ...
        case POT_SLEEPING:
            ...
            if (sleep_monst(mon, rnd(12), POTION_CLASS)) {
                pline(_("%s falls asleep."), Monnam(mon));
                slept_monst(mon);
            }
            break;
        case POT_PARALYSIS:
        ...
        case POT_WATER:
            if (mon_hates_blessings(mon) /* undead or demon */
                ...
                if (obj->blessed) {
                    ...
                    angermon = FALSE;
                    if (canseemon(mon))
                        pline(_("%s looks healthier."), Monnam(mon));
                    healmon(mon, d(2, 6), 0);
...
potionbreathe(struct obj *obj)
{
    ...
    case TOWEL:
        pline(_("Some vapor passes harmlessly around you."));
        break;
    case POT_RESTORE_ABILITY:
    case POT_GAIN_ABILITY:
        if (obj->cursed) {
    ...
    case POT_SICKNESS:
        ...
        break;
    case POT_HALLUCINATION:
        You(_("have a momentary vision."));
        break;
    case POT_CONFUSION:
    case POT_BOOZE:
        if (!Confusion)
            You_feel(_("somewhat dizzy."));
    ...
    case POT_SPEED:
        if (!Fast)
            Your(_("knees seem more flexible now."));
...
dip_potion_explosion(struct obj *obj, int dmg)
{
    if (obj->cursed || obj->otyp == POT_ACID
        ...
        obj->in_use = 1;
        pline(_("%sThey explode!"), !Deaf ? _("BOOM!  ") : "");
...
potion_dip(struct obj *obj, struct obj *potion)
{
    ...
    if (potion == obj && potion->quan == 1L) {
        pline(_("That is a potion bottle, not a Klein bottle!"));
        return ECMD_OK;
    }
    if (obj == &hands_obj) {
        You(_("can't fit your %s into the mouth of the bottle!"),
            body_part(HAND));
        return ECMD_OK;
    }
    ...
    if (is_poisonable(obj)) {
        if (potion->otyp == POT_SICKNESS && !obj->opoisoned) {
            ...
            pline(_("A coating wears off %s."), the(xname(obj)));
            obj->opoisoned = 0;
            poof(potion);
            return ECMD_TIME;
        }
...
djinni_from_bottle(struct obj *obj)
{
    ...
    if (!Blind) {
        pline(_("In a cloud of smoke, %s emerges!"), a_monnam(mtmp));
    ...
    struct monst *mtmp) /* optional attacker whose heat triggered it */
{
    ...
    if (mon == &gy.youmonst) {
        ...
        if (mtmp2) {
            ...
            if (canspotmon(mon))
                pline(_("%s multiplies%s!"), Monnam(mon), reason);
        }
    }
    return mtmp2;
}
