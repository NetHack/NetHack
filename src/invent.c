...
STATIC_DCL void loot_classify(Loot *, struct obj *);
STATIC_DCL char *loot_xname(struct obj *);
...
void
tally_BUCX(struct obj *list, boolean by_nexthere, int *bcp, int *ucp, int *ccp, int *xcp, int *ocp)
{
    /* Future extensions:
...
count_contents(struct obj *container,
               ...
               boolean newdrop)     /* on floor, but hero-owned items haven't been marked
