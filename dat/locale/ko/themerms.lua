...
function make_dig_engraving(data)
   ...
   if (tx == 0 and ty == 0) then
      ...
      if (tx < 0 or tx > 0) then
         dig = string.format(" %s으로 %i칸", (tx > 0) and "동쪽" or "서쪽", math.abs(tx));
      end
      if (ty < 0 or ty > 0) then
         dig = dig .. string.format(" %s으로 %i칸", (ty > 0) and "남쪽" or "북쪽", math.abs(ty));
      end
   end
   des.engraving({ coord = pos, type = "burn", text = "파라" .. dig });
end
