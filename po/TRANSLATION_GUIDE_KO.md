...
| 메시지 유형 | 문체 | 예시 |
|------------|------|------|
| **사용자 질문/프롬프트** | 존댓말 (-ㅂ니까/-세요) | "무엇을 버리겠습니까?", "선택하세요" |
| **게임 상황 설명** | 반말 (-다) | "배가 고프다.", "%s{을/를} 때렸다." |
| **상점 주인 대화** | 반존대 (-요) | "나가기 전에 계산해 주세요.", "환영해요!" |
| **일반 NPC 대화** | 반말 (캐주얼) | "안녕.", "따라와." |
| **시스템 안내** | 존댓말 | "버그는 <%s>로 보고해 주세요." |

#### 사용자 질문 (존댓말)
플레이어에게 선택이나 입력을 요청하는 프롬프트는 존댓말을 사용합니다.
...
#### 상점 주인 대화 (반존대)
상점 주인이 손님(플레이어)에게 하는 말은 반존대(-요)를 사용합니다.
...
#### 일반 NPC 대화 (반말/캐주얼)
일반 NPC가 플레이어에게 하는 말은 캐주얼한 반말을 사용합니다.
...
```
1. 원본 영어 파일 확인
   cat dat/help      # 원본 영어 도움말

2. 한국어 번역 파일 생성/편집
   vi dat/locale/ko/help

3. 빌드 후 적용 확인
   cd /home/jinhong_kim/workspace/HanNetHack && make
   ./src/nethack     # ? 키로 도움말 확인
```

#### 번역 규칙

1. **형식 유지**: 원본의 줄바꿈, 공백, 정렬 유지
2. **문체**: '-다' 체 사용 (해요체 X)
3. **명령어**: 영어 명령어는 그대로 유지
   ```
   원본: Press 'h' for help
   번역: 도움말을 보려면 'h'를 누르다
   ```
...
|------|---------|
| help | 일반 도움말, 초보자용 설명 |
| hh | 짧은 도움말, 한 화면에 맞춰야 함 |
| cmdhelp | 명령어 설명, 정렬 유지 중요 |
| keyhelp | 키 바인딩, 포맷 유지 |
| opthelp | 옵션 설명, 기술 용어 주의 |
| wizhelp | 마법사 모드 전용 |
...
2. 한국어 번역 파일 생성/편집
   vi dat/locale/ko/rumors.tru

3. 빌드 & 데이터 재생성
   cd /home/jinhong_kim/workspace/HanNetHack && make

4. 게임에서 확인
   ./src/nethack    # 쿠키 읽기 등으로 확인
```
...
1. **소문 번역**: 게임 힌트는 정확하게, 유머는 한국적으로 변환 가능
2. **묘비 번역**: 짧고 인상적인 문구, 한국 정서에 맞게 의역 가능
3. **수량 맞추기**: 원본과 동일한 수의 항목 유지
...
```lua
-- 원본
des.message("Welcome to the Dungeons of Doom!")

-- 번역
des.message("운명의 던전에 오신 것을 환영합니다!")
```
